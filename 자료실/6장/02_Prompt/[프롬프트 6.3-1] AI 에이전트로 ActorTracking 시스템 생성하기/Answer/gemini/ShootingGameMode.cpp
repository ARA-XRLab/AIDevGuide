...
void AShootingGameMode::BeginPlay()
{
	...
	GetWorldTimerManager().SetTimer(ActorTrackingTimerHandle, this, &AShootingGameMode::TrackActors, 5.0f, true);
}
...
void AShootingGameMode::TrackActors()
{
	...
	for (auto& Elem : ActorCounts)
	{
		LogContent += FString::Printf(TEXT("[%s] %s: %d\n"), *Timestamp, *Elem.Key, Elem.Value);
	}

	FString FilePath = FPaths::ProjectSavedDir() / TEXT("Logs/ActorTracking.log");
	FFileHelper::SaveStringToFile(LogContent, *FilePath, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), FILEWRITE_Append);
}
